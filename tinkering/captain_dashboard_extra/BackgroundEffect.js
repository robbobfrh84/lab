...
class BackgroundEffect {
  ...
  moveRacers() {
    this.lines.forEach( (l,i) => {
      ...
      if (l.x > l.lineW+window.innerWidth || isNaN(l.x)) {
        this.lines[i] = l = this.newRacers()
      }
      l.x += l.speed
      let color = this.c.lGrad({
        gradient: [ l.x, 0, l.x + (l.lineW*this.speed), 0],
        colors: l.colors
      })
      this.c.line(l.x, l.y, l.x + (l.lineW*this.speed), l.y, color, l.lineS, "round")
    })
  }
  ...
  moveLightSpeed() {
    this.lines.forEach( (l, i) => {
      // 🗺 Check for new locaiton
      if (l.dist-l.lineW > this.longer/2) {
        this.lines[i] = l = this.newLightSpeed()
      }
      l.dist += l.speed
      l.x = (l.dist * Math.cos(l.angle)) + this.cx
      l.y = (l.dist * Math.sin(l.angle)) + this.cy
      const l2x = ((l.dist+(l.lineW*this.speed)) * Math.cos(l.angle)) + this.cx
      const l2y = ((l.dist+(l.lineW*this.speed)) * Math.sin(l.angle)) + this.cy
      let color = this.c.lGrad({
  ...
  angles() {
    ...
    const lineS = this.lineS() // _rand(1,6)
    let colors;
    if (this.setting === "night") {
      const [r,g,b] = this.colorsNight()
